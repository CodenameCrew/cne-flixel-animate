...
class FlxAnimate extends FlxSprite
{
	...
	function checkRenderTexture():Bool
	{
		...
		return false;
		#else
		return isAnimate && useRenderTexture && (layer != null || alpha != 1 || shader != null || (blend != null && blend != NORMAL));
		#end
	}
	...
	function drawAnimate(camera:FlxCamera):Void
	{
		...
		if (willUseRenderTexture)
		{
			...
			if (layer != null)
				layer.drawPixels(this, camera, _renderTexture.graphic.imageFrame.frame, framePixels, matrix, colorTransform, blend,
					antialiasing, shaderEnabled ? shader : null, wrapMode);
			else
				camera.drawPixels(_renderTexture.graphic.imageFrame.frame, framePixels, matrix, colorTransform, blend, antialiasing,
					shaderEnabled ? shader : null, wrapMode);
		}
		else
		#end
		{
			timeline.draw(camera, matrix, colorTransform, blend, antialiasing, shaderEnabled ? shader : null, wrapMode);
		}
	}
